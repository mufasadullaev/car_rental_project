...
function getByOwner($tableName, $ownerId) {
    global $conn; // Ensure that $conn (database connection) is accessible

    // Adjust the SQL query to filter by owner_id
    $query = "SELECT * FROM `$tableName` WHERE owner_id = '$ownerId'";
    $result = mysqli_query($conn, $query);
    return $result;
}
